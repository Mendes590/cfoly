...
const LAND_CSS = `
  *, *::before, *::after { box-sizing: border-box; }
  ...
  @media(max-width:760px) {
    ...
    .secPad  { padding:52px 24px !important }
    ...
    <div style={{
      ...
      <div className="navPad" style={{ maxWidth: W, margin: "0 auto", height: 62, display: "flex", alignItems: "center", gap: 32 }}>
        ...
        <nav className="navLinks" style={{ flex: 1 }}>
          {NAV_LINKS.map(({ label, id }) => (
            <span key={id} className="lLink" style={{ fontSize: 13.5, color: C.textS, fontWeight: 500 }}
              onClick={() => scrollTo(id)}
            >{label}</span>
          ))}
        </nav>
      ...
      {menuOpen && (
        <div className="mobileMenu open" style={{
          ...
          {NAV_LINKS.map(({ label, id }) => (
            <button key={id} style={{ textAlign: "left", background: "none", border: "none", padding: "10px 4px", fontSize: 15, color: C.textS, fontFamily: "inherit", cursor: "pointer", fontWeight: 500, borderBottom: `1px solid ${C.border}` }}
...
function HowItWorks() {
  return (
    <div id="how-it-works" style={{ background: C.bgDeep, borderTop: `1px solid ${C.border}`, borderBottom: `1px solid ${C.border}` }}>
...
/* ════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════ */
const FEATURES = [
  { color: C.blueL, title: "Real-time financial dashboards",   desc: "Revenue, expenses, margin, and runway unified in one always-current view. No manual updates.",              icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="3" width="7" height="9"/><rect x="14" y="3" width="7" height="5"/><rect x="14" y="12" width="7" height="9"/><rect x="3" y="16" width="7" height="5"/></svg> },
...
function Features() {
  return (
    <div id="features" style={{ background: C.bgDeep, borderTop: `1px solid ${C.border}`, borderBottom: `1px solid ${C.border}` }}>
      <div className="secPad" style={{ maxWidth: W, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 52 }}>
